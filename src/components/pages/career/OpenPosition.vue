...
const getCareerData = async () => {
  ...
  try {
    ...
    if (!Array.isArray(response)) {
      throw new Error("Received invalid data for career positions");
    }
    ...
    error.value = err.message || "Failed to fetch career data.";
...
<template>
  <div class="px-[56px] md:px-[112px]">
    <div class="flex flex-col gap-9 items-center mb-[60px]">
      <span
        class="bg-gray-100 dark:bg-[#121212] px-4 py-2 rounded-lg text-xs font-medium inline mt-[50px]"
      >
        Open Position
      </span>
      <h1
        class="font-raleway text-[25px] md:text-[45px] font-bold text-gray-800 dark:text-gradient text-center"
      >
        Explore Opportunities,
        <span class="text-gradient">Shape your Career</span>
      </h1>
      <p
        class="font-raleway text-[13px] md:text-[16px] font-semibold text-gray-600 dark:text-white"
      >
        See available positions and find a role where your skills and passions
        align with our mission.<br />
      ...
      >
        ...
        >
          ...
          >
            ...
            <p
              class="text-xs text-justify text-gray-600 break-all dark:text-gray-400"
            >
              {{ position.ket_lowong.ringkasan }}
            </p>
            <div
              class="absolute bottom-0 flex items-center justify-center -right-1"
            >
      ...
      <div class="mb-6 space-y-4 text-left md:mb-0">
        ...
        <div class="hidden md:block">
          <p class="max-w-xl text-white/90">
            Join our innovative team where professionalism meets creativity.
            Experience a modern work environment with friendly colleagues and
            flexible payment options.
          </p>
        </div>
        <div class="block md:hidden">
          <p class="max-w-xl text-white/90">
            Our company is a professional company, with friendly service, modern
            homes and interest-free payments
          </p>
        </div>
      </div>
      <div>
        <Button
          class="relative w-40 h-10 flex-nowrap button-ping rounded-full"
          @click="$router.push({ name: 'contact' })"
        >
          <div class="flex items-center justify-center gap-2">
            <span class="text-base">Let's Chat</span>
            <svg
...
<style scoped>
.frame-section-bg {
  background-image: url(@/assets/frameworktogether.png),
    linear-gradient(to bottom, #2f84ff, #0c57c3);
...
@keyframes pulse512 {
  0% {
    box-shadow: 0 0 0 0 rgba(4, 0, 255, 0.4);
  }

  70% {
    box-shadow: 0 0 0 10px rgba(4, 0, 255, 0);
  }

  100% {
    box-shadow: 0 0 0 0 rgba(4, 0, 255, 0);
  }
}
</style>
