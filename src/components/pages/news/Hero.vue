<template>
  <section class="relative py-16 px-2 dark:bg-black overflow-hidden">
    <img
    ...
    <div class="mb-12 text-center">
      <p
        class="inline-block px-4 py-2 text-sm font-medium text-blue-600 bg-blue-100 dark:bg-blue-800 dark:text-blue-200 rounded-md"
      >
        Blog
      </p>
      <h1
        class="mt-4 text-3xl font-semibold text-light-primary dark:text-white font-raleway"
      >
    ...
    <div class="relative max-w-6xl mx-auto overflow-hidden rounded-[20px]">
      <button
        @click="prevSlide"
      ...
      >
        <div
          v-for="(blog, index) in newsCarouselData"
          :key="index"
        ...
        >
          <div class="relative h-96">
            <img
              :src="
                blog.fotos?.length
                  ? `https://content.hexagon.co.id/storage/${blog.fotos[0].nama_foto}`
                  : ''
              "
              :alt="blog.judul_news"
            ...
            >
              <div class="mb-3 space-x-6 text-xs font-medium text-gray-200">
                <span class="uppercase">{{ blog.category_id }}</span>
                <svg
              ...
              <div class="sm:max-w-[400px] max-w-full">
                <p class="mt-1 text-xs text-white sm:text-sm line-clamp-2">
                  {{ blog.ket_news }}
                </p>
              </div>
            </div>
      ...
      <div class="absolute flex gap-2 right-20 bottom-4">
        <button
          v-for="(_, index) in newsCarouselData"
          :key="index"
          @click="goToSlide(index)"
          class="w-2 h-2 transition-all rounded-full"
          :class="currentIndex === index ? 'bg-[#157EFB] w-4' : 'bg-white'"
        ></button>
      </div>
    </div>
  </section>
...
const getCarouselData = async () => {
  ...
  if (data.error) {
    ...
    newsCarouselData.value = data.data;
  }
};

const nextSlide = () => {
  if (newsCarouselData.value.length > 0) {
    currentIndex.value =
      (currentIndex.value + 1) % newsCarouselData.value.length;
  }
};
