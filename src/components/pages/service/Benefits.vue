        ...
        <div class="flex flex-col gap-9 mb-3 md:mb-[78px] mt-[36px]">
            <div class="flex flex-col lg:flex-row gap-[50px] mb-[18px] mt-10">
                ...
                <div class="px-[36px] md:px-[112px] mb-6 lg:w-1/2">
                    ...
                    <h1 class="text-[30px] md:text-[38px] font-raleway tracking-[-0.64px] leading-9 font-bold text-gray-800 dark:text-white mb-4 mt-[36px]">
                        Why Choose <br class="md:hidden"> <span class="text-gradient">Hexagon inc</span> ?
                    </h1>
                    <!-- Deskripsi Benefits -->
                    <p class="text-xs text-justify text-gray-600 dark:text-white">
                        Discover the advantages of partnering with us—tailored solutions, dedicated support, and proven results that set us apart.
                    </p>
...
<div class="divide-y divide-neutral/20">
    ...
    class="group w-full flex items-center gap-x-2 text-start py-4 px-5 font-semibold text-base dark:text-white text-black hover:text-blue-600 dark:hover:bg-slate-900 hover:bg-blue-50 dark:hover:text-blue-600  transition-all duration-200"
    :aria-expanded="isActive(index)"
    :class="{
        'bg-blue-50 dark:bg-slate-900 rounded text-blue-600 dark:text-blue-600': isActive(index)
    }"
    >
    <span
    ...
    {{ item.title }}
      ...
      >
        <div v-show="isActive(index)" class="overflow-hidden bg-blue-50 rounded-b  dark:bg-slate-900 px-5 pb-4">
